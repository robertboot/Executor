    ...
    try {
      ...
      try {
        const { data } = await supabase
          .from('items')
          .select('id, inventory_id, name, category, location, value_amount, value_currency, created_at')
  ...
  return (
    ...
    >
        ...
        <View style={styles.list}>
          {recent.map((it, idx) => (
            <Link key={it.id} href={`/(app)/inventory/${it.inventory_id}/item/${it.id}`} asChild>
              <Pressable style={[styles.row, idx > 0 && styles.rowDivider]}>
      ...
      {collections.length === 0 ? (
        <View style={styles.emptyCard}>
          <Text style={styles.emptyText}>Add a collection to group items.</Text>
        </View>
      ) : (
        <View style={styles.list}>
...
const styles = StyleSheet.create({
  ...
  brand: { fontSize: 30, fontWeight: '700', color: colors.ink, letterSpacing: 0.5 },
  gear: {
  ...
  primaryBtn: {
    backgroundColor: colors.forest, padding: 12, borderRadius: radius.md, alignItems: 'center',
  },
